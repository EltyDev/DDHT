...
style edited is default:
    font "fonts/VerilySerifMono.otf"
    kerning 8
    outlines [(10, "#000", 0, 0)]
    xpos gui.text_xpos
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Veuillez entrer votre nom", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("Nouvelle Partie") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Veuillez entrer votre nom", ok_action=Function(FinishEnterName)))

            else:

                textbutton _("Historique") action [ShowMenu("history"), SensitiveIf(renpy.get_screen("history") == None)]

                textbutton _("Sauvegarder") action [ShowMenu("save"), SensitiveIf(renpy.get_screen("save") == None)]

            textbutton _("Charger") action [ShowMenu("load"), SensitiveIf(renpy.get_screen("load") == None)]

            if _in_replay:

                textbutton _("Fin du Replay") action EndReplay(confirm=True)
            ...
            textbutton _("Paramètres") action [ShowMenu("preferences"), SensitiveIf(renpy.get_screen("preferences") == None)]
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Aide") action [Help("README.html"), Show(screen="dialog", message="Le fichier d'aide a été ouvert sur votre navigateur.", ok_action=Hide("dialog"))]
...
init python:
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="Erreur du fichier : \"characters/sayori.chr\"\n\nLe fichier est introuvable ou corrompu.",
                ok_action=Show(screen="dialog", message="Le fichier de sauvegarde est corrompu. Commencer une nouvelle partie.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="Il n'y a plus d'intérêt à sauvegarder.\nNe t'en fais pas, je ne vais nulle part.", ok_action=Hide("dialog"))
        else:
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    label _("Vitesse d'affichage")


                    bar value FieldValue(_preferences, "text_cps", range=180, max_is_zero=False, style="slider", offset=20)


                    label _("Vitesse de lecture auto")
...
screen history() tag menu:
    ...
    use game_menu(_("Historique"), scroll=("vpgrid" if gui.history_height else "viewport")):
