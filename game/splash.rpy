init python:
    menu_trans_time = 1
    splash_message_default = "Ce jeu n'est pas affilié à la Team Salvato\nni à la Team Yarashi."
    splash_messages = [
    "Tu es mon rayon de soleil,\nmon unique rayon de soleil.",
    "Tu m'as manqué.",
    "Joue avec moi.",
    "Ce n'est qu'un jeu, ou presque.",
    "Ce jeu ne convient pas aux enfants\nni aux personnes très sensibles ?",
    "sdfasdklfgsdfgsgoinrfoenlvbd",
    "null",
    "J'ai accordé l'Enfer à des enfants",
    "PM est mort pour ça.",
    "Ce n'était que partiellement de ta faute.",
    "Ce jeu ne convient pas aux enfants\nni aux personnes facilement démembrées.",
    "N'oublie pas d'effectuer une sauvegarde du fichier du personnage Monika."
    ]
...
label splashscreen:
    ...
    if not firstrun:
        if persistent.first_run:
            ...
            scene black
            menu:
                "Une précédente sauvegarde a été trouvée. Souhaiteriez-vous la supprimer et recommencer ?"
                "Oui, supprimer mes données existantes.":
                    "Suppression en cours des données de sauvegarde...{nw}"
                    python:
                        delete_all_saves()
                        renpy.loadsave.location.unlink_persistent()
                        renpy.persistent.should_save_persistent = False
                        renpy.utter_restart()
                "Non, reprendre là où j'étais.":
                    pass
    ...
    if not persistent.first_run:
        ...
        with Dissolve(1.0)
        pause 1.0
        "Ce jeu ne convient pas aux enfants\nni aux personnes sensibles."
        "Les personnes souffrant d'angoisse ou de dépression pourraient ne pas se sentir en sécurité en jouant à ce jeu. Pour voir le contenu dédié aux avertissements, veuillez aller sur : http://ddlc.moe/warning.html."
        menu:
            "En jouant à Doki Doki Literature Club, vous reconnaissez avoir plus de 13 ans, et vous consentez à être exposé à du contenu très dérangeant."
            "Je le reconnais.":
                pass
    ...
    if s_kill_early:
        ...
        show noise:
            alpha 0.1
        with Dissolve(1.0)
        show expression Text("Maintenant, tout le monde peut être heureux.", style="sayori_text"):
            xalign 0.8
            yalign 0.5
            alpha 0.0
            600
...
label after_load:
    ...
    if persistent.yuri_kill > 0 and persistent.autoload == "yuri_kill_2":
        ...
        stop music
        scene black
        "La sauvegarde n'a pas pu être chargée."
        ...
        if persistent.playername == "":
            ...
            m "Tu es si amusant, [persistent.playername]."
        $ renpy.utter_restart()
    else:
        if persistent.playthrough == 0 and not persistent.first_load and not config.developer:
            $ persistent.first_load = True
            call screen dialog("Conseil : Vous pouvez utiliser le bouton « Passer » afin\nd'avancer rapidement dans le texte que vous avez déjà lu.", ok_action=Return())
    return
...
label readonly:
    scene black
    "Ce jeu ne peut pas se lancer car vous tentez de le lancer depuis un emplacement de lecture seule."
    "Veuillez copier l'application DDLC sur votre bureau ou sur un autre emplacement accessible, puis réessayez."
    $ renpy.quit()
    return
